...
const childExitPromise = (child: ChildProcessPromise) => {
  ...
  return new Promise((resolve: any) => child.on('exit', resolve))
}

describe('ProcessSpawner', () => {
  it('kills processes', async function (): Promise<undefined> {
    this.timeout(250)
  ...
  it('enforces maxProcesses', async function (): Promise<undefined> {
    this.timeout(250)
    ...
    expect(() => handler.killOnExit(processes[1])).to.throw(
      'exceeded 1 simultaneous running processes, terminating all child processes'
    )
